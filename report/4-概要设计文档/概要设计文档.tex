...
% ===== 标题 =====
\begin{center}
    {\LARGE\bfseries 基于Pygame的2D西游记RPG小游戏\\[0.3cm]概要设计文档}\\[1cm]
\end{center}
...
本文档适用于以下读者：
\begin{itemize}
    \item 开发人员：理解系统架构和模块职责
    \item 测试人员：了解系统结构以设计测试用例
    \item 课程教师：了解项目的设计方案
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth, height=0.85\textheight, keepaspectratio]{figures/component_diagram.png}
    \caption{系统分层架构图}
...
\begin{longtable}{>{\centering\arraybackslash}m{2cm}>{\centering\arraybackslash}m{3cm}>{\centering\arraybackslash}m{6cm}}
\caption{系统分层职责} \label{tab:layers} \\
\toprule
\textbf{层级} & \textbf{模块} & \textbf{职责} \\
\midrule
\endfirsthead
...
\begin{enumerate}[label=\arabic*.]
    \item \textbf{核心模块}：Game主类、SceneManager场景管理器
    \item \textbf{角色模块}：ActorBase基类、Player、BattlePlayer、NPCBase、EnemyBase
    \item \textbf{系统模块}：DialogSystem、PlayerStats、SoundSystem、CollisionSystem
    \item \textbf{场景模块}：VillageScene、TempleScene、BattleScene、SettlementScene
    \item \textbf{工具模块}：TiledScene、FadeScene
\end{enumerate}

\subsection{技术选型与依据}

\begin{longtable}{>{\centering\arraybackslash}m{2.5cm}>{\centering\arraybackslash}m{3cm}>{\centering\arraybackslash}m{5.5cm}}
\caption{技术选型} \label{tab:tech} \\
\toprule
\textbf{技术} & \textbf{版本} & \textbf{选型依据} \\
\midrule
\endfirsthead
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figures/use_case.png}
    \caption{系统用例图}
    \label{fig:usecase}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth, height=0.85\textheight, keepaspectratio]{figures/class_diagram.png}
    \caption{核心类关系图}
    \label{fig:class}
\end{figure}
...
\begin{longtable}{>{\centering\arraybackslash}m{2.5cm}>{\centering\arraybackslash}m{2.5cm}>{\centering\arraybackslash}m{5cm}}
\caption{核心接口说明} \label{tab:interfaces} \\
\toprule
\textbf{接口名称} & \textbf{所属模块} & \textbf{功能说明} \\
\midrule
\endfirsthead
...
set\_current\_scene() & SceneManager & 切换当前场景，支持渐变过渡 \\
handle\_events() & SceneBase & 处理用户输入事件 \\
update() & SceneBase & 更新场景状态 \\
draw() & SceneBase & 渲染场景画面 \\
on\_enter() & SceneBase & 场景进入时初始化 \\
on\_exit() & SceneBase & 场景退出时清理 \\
start\_attack() & BattlePlayer & 开始攻击动画 \\
get\_attack\_rect() & BattlePlayer & 获取攻击范围 \\
take\_damage() & ActorBase & 接受伤害处理 \\
chase\_and\_attack() & EnemyBase & 敌人追击AI逻辑 \\
play\_music() & SoundSystem & 播放背景音乐，支持渐弱切换 \\
add\_elder\_hp() & PlayerStats & 增加血量属性 \\
add\_tang\_attack() & PlayerStats & 增加攻击力属性 \\
\end{longtable}
...
\subsubsection{PlayerStats 属性系统}

PlayerStats 是玩家属性的持久化存储类，采用单例模式，通过引用传递给所有场景。

\begin{lstlisting}[language=Python,caption={PlayerStats类定义}]
class PlayerStats:
    # 可调节的配置值
    BASE_HP = 60              # 初始血量
...
\subsubsection{ActorBase 角色基类}

\begin{lstlisting}[language=Python,caption={ActorBase类定义}]
...
\subsubsection{BattlePlayer 战斗玩家}

\begin{lstlisting}[language=Python,caption={BattlePlayer类定义}]
...
\subsubsection{EnemyBase 敌人基类}

\begin{lstlisting}[language=Python,caption={EnemyBase类定义}]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth, height=0.85\textheight, keepaspectratio]{figures/sequence_attribute.png}
    \caption{属性系统时序图}
    \label{fig:seq_attr}
\end{figure}
...
\begin{enumerate}[label=\arabic*.]
    \item \textbf{创建阶段}：Game主类创建PlayerStats单例
    \item \textbf{传递阶段}：通过构造函数传递给各场景
    \item \textbf{更新阶段}：VillageScene更新属性（NPC交互加成）
    \item \textbf{读取阶段}：BattleScene读取属性（创建BattlePlayer）
    \item \textbf{回写阶段}：BattleScene写回战斗结果
\end{enumerate}
...
\subsubsection{Game主类}

Game类是游戏入口，负责初始化Pygame、创建显示窗口、启动游戏主循环。

\begin{lstlisting}[language=Python,caption={Game主循环伪代码}]
def main():
    pygame.init()
...
\subsubsection{SceneManager 场景管理器}

SceneManager 负责场景的注册、切换和渐变过渡。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth, height=0.85\textheight, keepaspectratio]{figures/sequence_scene.png}
    \caption{场景切换时序图}
    \label{fig:seq_scene}
\end{figure}
...
\begin{lstlisting}[language=Python,caption={场景切换伪代码}]
...
\subsubsection{Player 探索玩家}

玩家在村庄和寺庙场景中使用，支持WASD/方向键移动，具备碰撞检测。

\begin{lstlisting}[language=Python,caption={Player移动算法}]
...
\begin{lstlisting}[language=Python,caption={攻击范围计算}]
...
\subsubsection{EnemyBase 敌人AI}

敌人AI采用状态机模式，包含站立、追击、攻击三种状态。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{figures/state_enemy_ai.png}
    \caption{敌人AI状态机}
    \label{fig:enemy_ai}
\end{figure}

\begin{lstlisting}[language=Python,caption={敌人追击AI算法}]
...
\subsubsection{SoundSystem 音效系统}

音效系统提供BGM播放和渐弱切换功能。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/algorithm_music.png}
    \caption{音乐渐弱切换算法}
    \label{fig:algorithm_music}
\end{figure}

\begin{lstlisting}[language=Python,caption={音乐渐弱切换算法}]
...
\subsubsection{DialogSystem 对话系统}

\begin{lstlisting}[language=Python,caption={对话系统伪代码}]
...
\subsubsection{VillageScene 村庄场景}

村庄场景是游戏的中心枢纽，玩家在此与NPC交互获取属性加成。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth, height=0.85\textheight, keepaspectratio]{figures/activity_village.png}
    \caption{村庄场景活动图}
    \label{fig:activity_village}
\end{figure}

核心流程：
\begin{enumerate}[label=\arabic*.]
    \item 加载village1.tmx地图和障碍物
    \item 从TMX对象层加载NPC位置
    \item 玩家WASD移动，检测碰撞
    \item 靠近NPC按空格触发对话
    \item 对话结束后给予属性加成
...
\subsubsection{BattleScene 战斗场景}

战斗场景实现完整的实时战斗系统，包括波次系统和HUD显示。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth, height=0.85\textheight, keepaspectratio]{figures/activity_battle.png}
    \caption{战斗系统活动图}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth, height=0.85\textheight, keepaspectratio]{figures/sequence_battle.png}
    \caption{战斗交互时序图}
...
\begin{lstlisting}[language=Python,caption={波次切换算法}]
def update(self):
    if self.battle_over:
        return
...
\subsubsection{TiledScene TMX地图渲染器}

\begin{lstlisting}[language=Python,caption={TMX地图加载伪代码}]
class TiledScene:
...
\subsubsection{FadeScene 渐变效果}

\begin{lstlisting}[language=Python,caption={渐变效果伪代码}]
class FadeScene:
    alpha_step = 20
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/algorithm_collision.png}
    \caption{碰撞检测算法流程}
    \label{fig:collision}
\end{figure}
...
\begin{longtable}{>{\centering\arraybackslash}m{2.5cm}>{\centering\arraybackslash}m{3cm}>{\centering\arraybackslash}m{5.5cm}}
\caption{碰撞检测类型} \label{tab:collision} \\
\toprule
\textbf{检测类型} & \textbf{碰撞框} & \textbf{说明} \\
\midrule
\endfirsthead
...
玩家-障碍物 & foot\_rect & 脚部区域（50\%宽×35\%高） \\
玩家-NPC & inflate(20,20) & 扩大检测范围 \\
玩家-怪物 & inflate(40,40) & 扩大检测范围 \\
攻击-敌人 & attack\_rect & 方向相关的挥砍范围 \\
敌人-玩家 & attack\_rect & 敌人攻击范围 \\
\end{longtable}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth, height=0.85\textheight, keepaspectratio]{figures/algorithm_wave.png}
    \caption{波次切换算法流程}
    \label{fig:wave}
\end{figure}
...
波次系统设计：
\begin{itemize}
    \item 总波次数：TOTAL\_WAVES = 3
    \item 波次间隔：WAVE\_DELAY = 90帧（约2.25秒）
    \item 每波生成1个Cattle敌人
    \item 击败当前波敌人后开始下一波计时
    \item 全部击败即为战斗胜利
\end{itemize}
...
\begin{longtable}{>{\centering\arraybackslash}m{2.5cm}>{\centering\arraybackslash}m{2.5cm}>{\centering\arraybackslash}m{5cm}}
\caption{依赖库说明} \label{tab:dependencies} \\
\toprule
...
\begin{enumerate}[label=\arabic*.]
    \item 安装Python 3.0及以上版本
    \item 安装依赖库：\texttt{pip install pygame pytmx}
    \item 确保resource目录下的资源文件完整
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/data_flow.png}
    \caption{数据流图}
    \label{fig:data_flow}
\end{figure}
